...
group_by = st.sidebar.radio("Agrupar por", ["Mes", "Equipo", "Sector", "Especie"])
...
# SPECIES BREAKDOWN
# =============================================================================
if especies_selected and len(especies_selected) > 0:
    with st.expander("Desglose por Especie", expanded=True):
...
# ADVANCED METRICS
# =============================================================================
with st.expander("Metricas Avanzadas"):
    tab_trend, tab_heatmap, tab_top = st.tabs(["Tendencia", "Heatmap Cumplimiento", "Top/Bottom Sectores"])

    with tab_trend:
        trend_gran = st.radio("Granularidad", ["Semanal", "Mensual"], horizontal=True, key="trend_gran")
        if trend_gran == "Semanal":
            df_s_f['periodo'] = df_s_f['fecha'].dt.isocalendar().week.astype(int).apply(lambda w: f"Sem {w}")
            df_e_f['periodo'] = df_e_f['fecha'].dt.isocalendar().week.astype(int).apply(lambda w: f"Sem {w}")
            period_key = 'periodo'
        else:
            df_s_f['periodo'] = df_s_f['mes']
            df_e_f['periodo'] = df_e_f['mes']
            period_key = 'periodo'

        t_sol = df_s_f.groupby(period_key)['m3'].sum().reset_index()
        t_sol.columns = [period_key, 'Solicitado']
        t_ejec = df_e_f.groupby(period_key)['m3'].sum().reset_index()
...
# DRILL DOWN
# =============================================================================
st.subheader("Drill Down por Equipo y Sector")

drill_col1, drill_col2 = st.columns([1, 2])

with drill_col1:
    st.write("**Filtro rapido**")
    drill_equipo = st.selectbox("Equipo", ["Todos"] + sorted(all_equipos))
    drill_sector = "Todos"

    if drill_equipo != "Todos":
        sectoros_eq = sorted(df_s_f[df_s_f['equipo_num'] == drill_equipo]['sector_nom'].unique().tolist())
        drill_sector = st.selectbox("Sector", ["Todos"] + sectoros_eq)
...
with exp1:
    st.download_button("Descargar CSV", csv, "riegos_comparativa.csv", "text/csv")
...
with exp2:
    ...
    buffer.seek(0)
    st.download_button("Descargar Excel", buffer.getvalue(), "riegos_comparativa.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
